...
export function ShowCard({ show }: { show: Show }) {
  const status = isUnconfirmedValue(show.status) ? "Archive" : show.status;
  ...
  return (
    <article className="archive-frame grid gap-6 p-6 md:grid-cols-[180px_1fr_auto] md:items-start md:p-7">
      ...
      <div>
        ...
        {show.alternateSourceUrl && <a href={show.alternateSourceUrl} target="_blank" rel="noopener noreferrer" className="mt-4 inline-flex text-xs font-black uppercase tracking-[0.18em] text-zinc-500 transition hover:text-gold">Additional Source</a>}
