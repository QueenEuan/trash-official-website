...
export function AlbumCard({ album }: { album: Album }) {
  ...
  return (
    <Link href={`/music/${album.slug}`} className="group grid gap-5 border border-white/10 bg-[linear-gradient(135deg,rgba(255,255,255,0.045),rgba(0,0,0,0.2))] p-4 transition hover:-translate-y-1 hover:border-gold/70 md:grid-cols-[190px_1fr]">
      ...
      <div className="self-center">
        ...
        <div className="mt-5 flex flex-wrap items-center gap-4">
          <p className="text-xs font-bold uppercase tracking-[0.14em] text-zinc-500">{album.trackCount} tracks</p>
          <span className="text-xs font-black uppercase tracking-[0.18em] text-gold transition group-hover:text-white">View Album</span>
        </div>
      </div>
    </Link>
  );
