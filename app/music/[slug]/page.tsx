    ...
    <div className="page-shell">
      ...
      <div className="mt-16 grid gap-10 lg:grid-cols-[1.1fr_0.9fr]">
        <section className="space-y-6 text-lg leading-8 text-zinc-300">
          <h2 className="kicker">Album Notes</h2>
          {description.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}
        </section>

        <aside className="archive-frame p-6">
          <h2 className="kicker">Release Info</h2>
          <dl className="mt-5 space-y-4">
            {year && <div><dt className="text-xs uppercase tracking-[0.16em] text-zinc-500">Year</dt><dd className="mt-1 text-xl font-bold text-white">{year}</dd></div>}
            <div><dt className="text-xs uppercase tracking-[0.16em] text-zinc-500">Type</dt><dd className="mt-1 text-xl font-bold text-white">{album.type}</dd></div>
            <div><dt className="text-xs uppercase tracking-[0.16em] text-zinc-500">Track Count</dt><dd className="mt-1 text-xl font-bold text-white">{album.trackCount}</dd></div>
            {releaseDate && <div><dt className="text-xs uppercase tracking-[0.16em] text-zinc-500">Date</dt><dd className="mt-1 text-xl font-bold text-white">{releaseDate}</dd></div>}
          </dl>
          {credits.length > 0 && <div className="mt-7 border-t border-white/10 pt-6">
            <h3 className="kicker">Credits</h3>
          ...
          {links.length > 0 && <div className="mt-7 border-t border-white/10 pt-6">
            <h3 className="kicker">Streaming</h3>
      ...
      {tracks.length > 0 && <section className="mt-16 archive-frame p-5 md:p-7">
        <h2 className="kicker">Tracklist</h2>
      ...
      <section className="mt-16">
        <h2 className="kicker">Discography</h2>
        <div className="mt-6 grid gap-6">
          {albums.filter((item) => item.slug !== album.slug).map((item) => <AlbumCard key={item.slug} album={item} />)}
        </div>
      </section>
    </div>
  );
